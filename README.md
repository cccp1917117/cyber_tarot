...
- `tarot_decks` / `tarot_cards` 分开保存套牌与单牌，允许同一原型在不同套牌中有不同图像和解释。
- `media_assets` 只接收 JPG/JPEG，保存相对存储键、尺寸、字节数和 SHA-256。图片本体不塞进数据库。
...
```text
prompts/moonlit-door.jpg
tarot/rider-waite/major-00.jpg
```
